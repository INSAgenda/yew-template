...
pub(crate) fn text_part_to_code(
    text_part: &TextPart,
    opts: &mut Vec<String>,
...
pub(crate) fn attr_to_code(
    ...
    match text_parts.len() {
        ...
        _ => {
            ...
            Some(format!(
                "{name}={{format!({format_literal}, {format_args})}}"
            ))
        }
    }
}
...
pub(crate) fn element_to_code(
    ...
    if el.self_closing && !el.close_attrs.is_empty() {
        abort!(
            args.path_span,
            "Self-closing tags cannot have closing attributes"
        );
    }
    ...
    let mut content = el
        ...
        .map(|p| {
            p.part
                .into_code(depth + 1, &mut inner_opts, &mut inner_iters, args)
        })
        .collect::<Vec<_>>()
        .join("");
    inner_opts.sort();
    ...
    content = match name == "virtual" {
        true => {
            if !f_open_attrs.is_empty() || !f_close_attrs.is_empty() {
                abort!(
                    args.path_span,
                    "Virtual elements cannot have attributes (found {:?} and {:?})",
                    f_open_attrs,
                    f_close_attrs
                );
            }
    ...
    match opt {
        true => {
            let left = inner_opts
                .iter()
                .map(|id| format!("Some(macro_produced_{id})"))
                .collect::<Vec<_>>()
                .join(", ");
            let right = inner_opts
                .iter()
                .map(|id| {
                    args.get_val(id, &mut Vec::new(), &mut Vec::new(), args)
                        .to_string()
                })
                .collect::<Vec<_>>()
                .join(", ");
    ...
    match iter {
        true => {
            let before = inner_iters
                ...
                .map(|id| {
                    format!(
                        "let mut macro_produced_{id} = {};",
                        args.get_val(id, &mut Vec::new(), &mut Vec::new(), args)
                    )
                })
                .collect::<Vec<_>>()
                .join("");
            let left = inner_iters
                .iter()
                .map(|id| format!("Some(macro_produced_{id})"))
                .collect::<Vec<_>>()
                .join(", ");
            let right = inner_iters
                .iter()
                .map(|id| format!("macro_produced_{id}.next()",))
                .collect::<Vec<_>>()
                .join(", ");
            content = content.replace('\n', "\n        ");
            content = format!(
    ...
    if let Some(mut present_if) = present_if {
        ...
        if !present_if.starts_with(&args.config.variable_bounds.0)
            || !present_if.ends_with(&args.config.variable_bounds.1)
        {
            abort!(args.path_span, "present_if attribute must be a variable");
        }
        present_if = present_if[args.config.variable_bounds.0.len()
            ..present_if.len() - args.config.variable_bounds.1.len()]
            .to_string();
